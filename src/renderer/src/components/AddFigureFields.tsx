...
import type { Accessory, FigureLook, HairStyle, RoomKey } from '@shared/figures';
  ...
  return (
    <div className="add-figure__fields">
      <DSField label="Name" htmlFor="figure-name" error={errors.name}>
        <DSInput id="figure-name" value={values.name} onChange={(value: string): void => setField('name', value)} placeholder={NAME_PLACEHOLDER} maxLength={NAME_MAX_LENGTH} />
      </DSField>
      <DSField label="Job" htmlFor="figure-job" error={errors.job}>
        <DSInput id="figure-job" value={values.job} onChange={(value: string): void => setField('job', value)} placeholder={JOB_PLACEHOLDER} maxLength={JOB_MAX_LENGTH} />
      </DSField>
      <DSField label="Department" htmlFor="figure-room" error={errors.room}>
        <DSSelect id="figure-room" value={values.room} options={roomOptions} onChange={(value: RoomKey): void => setField('room', value)} />
      </DSField>
      <DSField label="Hair" htmlFor="figure-hair">
        <DSSelect id="figure-hair" value={values.look.hairStyle} options={HAIR_STYLE_OPTIONS} onChange={(value: HairStyle): void => setLook('hairStyle', value)} />
      </DSField>
      <DSField label="Accessory" htmlFor="figure-accessory">
        <DSSelect id="figure-accessory" value={values.look.accessory} options={ACCESSORY_OPTIONS} onChange={(value: Accessory): void => setLook('accessory', value)} />
      </DSField>
      ...
      <DSField label="Role prompt" htmlFor="figure-prompt">
        <DSTextArea id="figure-prompt" value={values.rolePrompt} onChange={(value: string): void => setField('rolePrompt', value)} placeholder={PROMPT_PLACEHOLDER} />
      </DSField>
    </div>
  );
}
