...
export interface Typewriter {
  shown: string;
  isDone: boolean;
  /** Reveals the whole text at once (a click or Enter on the dialog). */
  skip: () => void;
}
...
export function useTypewriter(text: string): Typewriter {
  ...
  const isDone = shownLength >= total;

  useEffect((): (() => void) | undefined => {
    if (hasReducedMotion()) {
      setProgress({ text, shownLength: total });
      return undefined;
    }
  ...
  return { shown: characters.slice(0, shownLength).join(''), isDone, skip: (): void => setProgress({ text, shownLength: total }) };
}
