...
function greetingFor(figure: Figure): string {
  return `Hi boss! I'm ${figure.name}, the ${figure.job.toLowerCase()}. What do you need?`;
}
...
export function dialogGreeting(figure: Figure, latestRun: AgentRun | null): string {
  if (latestRun === null) return greetingFor(figure);
