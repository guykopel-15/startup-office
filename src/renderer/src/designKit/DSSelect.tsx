...
interface DSSelectProps<Value extends string> {
  id: string;
  value: Value;
...
export function DSSelect<Value extends string>({ id, value, options, onChange }: DSSelectProps<Value>): React.JSX.Element {
  const handleChange = (event: React.ChangeEvent<HTMLSelectElement>): void => onChange(event.target.value as Value);
  return (
    <select id={id} className="ds-input ds-select" value={value} onChange={handleChange}>
