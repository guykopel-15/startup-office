...
| Meeting room | No permanent figure. Where the team meets for sprint planning; the board lives in the HUD |
...
│   ├─ <HUD>            bottom bar: quests, sprint board, chat history, chat │
...
`figure:clicked`; React emits `figure:says` (a line to show over a figure) and `sprint:closed` (confetti). Phaser mirrors figure state and run output from `floorsStore` / `runsStore` / `sprintsStore`.
...
| Camera | Fits the office on resize when the user had not zoomed (the camera remembers the zoom of its last fit); otherwise keeps their zoom and re-clamps. Wheel zoom toward the pointer, left-drag pan. `GameCanvas` watches its host with a ResizeObserver, so the HUD growing (chat history, sprint board) refits the office too, not only window resizes |
...
| Movement | `navigation.ts` builds a walkable grid from whole wall segments and furniture (chairs and stools stay walkable; interior walls block the tile row on both sides of their line) and finds tile paths by breadth-first search; `FigureWalker` moves the feet along the path at a constant speed, cycles two leg frames and flips the sprite to face the way it goes. `FigureDirector` owns errands: every few seconds one figure that is not working (idle, done or error) walks to a random tile of its own room (never a colleague's seat), waits, and comes back; meeting spots are claimed from a free list, chairs first; a `meeting` state sends the figure to a spot around the meeting table (chairs first); work pulls it home. Typing only runs at the desk |
...
| XP | `src/shared/experience.ts`: a finished run pays 50 XP, a failed quest costs 10 (never below zero); level n starts at 50·n·(n−1) XP, capped at 20; a reached level is kept. `rewardRun` (store) pays out on every `done` event and emits `figure:rewarded` / `figure:hit` / `figure:leveledUp`; a level-up also posts "Level up! I am level N now." |
| Juice | `juice.ts`: "+50 XP" floats up in green, "-10" in red with a tint flash and a shake, a level-up bursts gold sparks and "LEVEL UP!"; confetti on sprint close. The name tag reads "Maya Lv2" from level 2; the dialog box header shows "Lv2 · 50 / 200 XP" |
| Sound | `sound.ts`: Web Audio oscillators, no asset files. Level-up arpeggio (square), failure thud (sawtooth), cheer chord on a closed sprint (triangle). The context is created on first play; `settingsStore.isMuted` (navbar speaker button) silences everything. Room loops and keyboard clatter are not built |
...
| Run fails to start | The quest fails, the figure replies "I couldn't start on that: <error>" and goes idle |
| Sprint plan empty or planning run fails | The planner says "I couldn't split that goal into tasks. Try a more concrete one.", everyone goes back to their desks, the sprint closes |
| Planning run stopped from the agent panel | The planner says "Meeting cancelled. Back to your desks.", the sprint closes |
| Planning run cannot start | Same as an empty plan, with the error in brackets |
