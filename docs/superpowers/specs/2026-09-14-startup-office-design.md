...
- Renderer: Vitest + Testing Library for the figures store, the Add figure dialog and modal, the Load repo dialog, the repo status chip, HUD (routing, replies through the event bridge, failed starts), DialogBox (typewriter, floor switch, focus), AgentPanel; pure unit tests for the pixel art (figure, typing and walk frames, badges), the bubble text, `routeTask`, the tasks store and the reply text, the sprints store, `parsePlan` and the planning prompt, the walkable grid and paths, `FigureWalker` with a fake scene, `useSprints` (plan, hold during meeting, empty plan, cancelled meeting, failed start) and `SprintBoard`. Main: the agent service's priority queue. Shared test helpers live in `src/renderer/src/test/`.
